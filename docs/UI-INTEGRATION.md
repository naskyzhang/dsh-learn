...
- **尺寸**：收起时只显示 196×68px 的猫和植物；点击后显示 280px 宽的学习卡片，包含课程摘要与完整技能树。卡片高度根据节点和资料数量自动增长，最多占据当前方向的可用视口空间，超出时内部滚动；空间不足会自动改为向上展开，不改变 DSH 的栏宽和主会话布局。
...
课程建立并写入 `LearnStore.nodes` 后，展开伙伴即可查看技能树：

- 卡片顶部使用模型在建课时语义概括的 `shortTitle`（不超过 8 个字），完整课程名仍保留在存储中；
- 按 `parent` 关系整理为前序层级树；根节点用菱形标记，子节点缩进并显示连接符；
- 每个节点显示简洁中文名、英文技能名、掌握度百分比和像素进度条；新课程中文标题由模型理解技能含义后语义概括为不超过 8 个字的完整短语，不承载冒号后的解释；客户端不机械截断标题；
- 点击节点的名称或进度区域，会通过当前 Session 的 `conversation.send()` 排入带有“学习卡片点击触发”标记的定向复习请求；请求携带节点 ID、双语名称及最多三条推荐资料，要求模型先出主动回忆或应用题，并在答题后调用 `learn_log_attempt`（`source: "card"`）记录结果。它是文献阅读完成前唯一的复习入口，且不会提前改变课程的 `learning` / `literature` 主阶段，也不会让该节点在文献后的首轮顺序复习中被跳过；
- 节点已挂学习资料时，显示最多两条可点击的“资料名”链接；
- 卡片为内容驱动高度；节点较少时不留空白，节点较多时限制在视口内并滚动；
- Host 只投影浏览器展示所需字段，资料 URL 再次过滤为 HTTP(S)，不传 drills、attempts、答案或完整复盘。
