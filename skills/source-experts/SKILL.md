...
4. **Link to skills.** Map each resource to the `nodeIds` it trains (from the curriculum). A resource can cover several nodes.
...
- Every resource links to at least one skill node, so practice can cite it.
- For programming domains, prefer real codebases the learner can read and run.

## After sourcing

Return to `practice-coach`: the practice tool surfaces linked resources as references for each drill.
